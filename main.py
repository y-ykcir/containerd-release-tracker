#!/usr/bin/env python3
"""
Containerd Release Tracker - Main CLI Entry Point

This tool tracks containerd releases, analyzes them using AI, and sends notifications.
"""
...
from src.tracker import ContainerdReleaseTracker
...
@click.pass_context
def cli(ctx, config):
    """Containerd Release Tracker - Monitor and analyze containerd releases."""
    ctx.ensure_object(dict)
    
    # Load configuration
    try:
        ctx.obj['config'] = load_config(config)
        ctx.obj['tracker'] = ContainerdReleaseTracker(ctx.obj['config'])
...
@click.option('--include-prerelease', is_flag=True,
              help='Include prerelease versions (beta, rc, etc.)')
@click.pass_context
...
def setup():
    ...
    click.echo("Setting up Containerd Release Tracker...")
